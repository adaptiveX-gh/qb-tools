// WeekRow — single accordion row for one week, with day grid input

function WeekRow({ week, onChange, onDuplicateDown, onSkipToggle, onClearWeek, isOpen, onToggleOpen, isLastCommitted }) {
  const ws = week.startDate;
  const days = Array.from({length:7}, (_,i) => {
    const d = new Date(ws); d.setDate(ws.getDate()+i); return d;
  });
  const totalH = week.hours.reduce((a,b)=>a+(parseFloat(b)||0),0);
  ...
  const setNote = (v) => onChange({ ...week, note: v });
    ...
    <div className={'week ' + (isOpen ? 'is-open ' : '') + (week.skipped ? 'is-skipped ' : '')}>
      <div className="week__head" onClick={onToggleOpen} role="button" aria-expanded={isOpen}>
        ...
        <span className={'week__chip week__chip--' + status}>
          {status === 'committed' && <Check size={12}/>}
          {status === 'ready' && <Check size={12}/>}
          {statusLabel}
          {status === 'committed' && week.qbId && <span style={{opacity:0.7, marginLeft:4}}>· {week.qbId}</span>}
        </span>
      ...
      {isOpen && (
        <div className="week__body">
          <div className="quickset">
            ...
            <select value={week.payCode || 'Regular'}
                    onChange={e => onChange({ ...week, payCode: e.target.value })}
          ...
          <div className="daygrid" role="grid">
            ...
            <div className="daygrid__row">
              <div className="daygrid__cell daygrid__cell--label">
                ...
                <div className="meta">{week.project} · {week.payCode || 'Regular Time'}</div>
              </div>
              {week.hours.map((h, i) => {
                const we = (i === 0 || i === 6);
          ...
          <div className="note-row">
            ...
            <input
              type="text"
              placeholder="Optional note for this week's invoice (e.g. on-site Tue–Wed)"
              value={week.note || ''}
              onChange={e => setNote(e.target.value)}
              disabled={week.committed}
            />
          </div>
